...
function projectionDef(s){if(s.projection==='lambert')return `+proj=lcc +lat_1=${s.truelat1} +lat_2=${s.truelat2} +lat_0=${s.refLat} +lon_0=${s.refLon} +datum=WGS84 +units=m +no_defs`;if(s.projection==='mercator')return `+proj=merc +lat_ts=${s.refLat} +lon_0=${s.refLon} +datum=WGS84 +units=m +no_defs`;if(s.projection==='polar'){const n=s.hemisphere!=='south';return `+proj=stere +lat_0=${n?90:-90} +lat_ts=${s.polarLat*(n?1:-1)} +lon_0=${s.refLon} +datum=WGS84 +units=m +no_defs`;}return null;}
...
function validate(domains,dx,dy,ratios,s){
  ...
  for(let i=0;i<domains.length;i++){
    ...
    if(ew<100||es<100)errors.push(`d${String(i+1).padStart(2,'0')} is smaller than the WRF recommended minimum of 100 × 100 grid points (${ew} × ${es}).`);
